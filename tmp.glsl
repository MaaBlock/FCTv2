...
layout (location = 1)  in vec3 vs2fs_position;
layout (location = 2)  in vec2 vs2fs_vectorCoord;
layout (location = 3) flat in float vs2fs_commandOffset;
layout (location = 4) flat in float vs2fs_commandSize;
...
void main() {
    ...
    ps_input.commandOffset = vs2fs_commandOffset;
    ps_input.commandSize = vs2fs_commandSize;
...
PixelOutput fct_user_main(PixelInput ps_input) {
    ...
    float i = ps_input.commandOffset;
    vec2 lastPos;
    ...
    float edge = i + ps_input.commandSize;
    while(i < edge) {
