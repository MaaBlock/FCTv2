...
int main()
{
    ...
    emjFont->translateGlyph(U'🥹');
    font->translateGlyph(U'草');
    font->translateGlyph(U'我');
    font->translateGlyph(U'靠');
    font->translateGlyph(U'操');
    font->translateGlyph(U'你');
    font->translateGlyph(U'妈');
    tp->addFont(abcFont);
    tp->addFont(font);
    tp->addFont(emjFont);
    leaveTextBoard->addFont(abcFont);
    leaveTextBoard->addFont(font);
    leaveTextBoard->addFont(emjFont);
    reviewport(ctx, wnd);
    ...
    while (wnd->isRunning())
    {
        ...
        std::u32string fps_u32str = std::u32string(fps_str.begin(), fps_str.end());
        if (needViewPort)
        {
            reviewport(ctx, wnd);
            needViewPort = false;
